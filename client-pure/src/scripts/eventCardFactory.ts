  ...
  switch (event.streamPath) {
    ...
    case "filings": {
      const [, companyNumber] = event.resource_uri.match(
        /^\/company\/([A-Z0-9]{8})\/filing-history/,
      );
      return {
        companyNumber,
    ...
    case "officers": {
      ...
      const description =
        "resigned_on" in event.data
          ? `Resigned on ${event.data.resigned_on}`
          : `Appointed on ${event.data.appointed_on}`;
    ...
    case "persons-with-significant-control": {
      ...
      const description =
        "ceased_on" in event.data
          ? `Ceased on ${event.data.ceased_on}`
          : `Notified on ${event.data.notified_on}`;
    ...
    case "disqualified-officers":
      ...
      const description = formatString(descriptionFormat, {
        companies: event.data.disqualifications?.flatMap((d) => d.company_names).join(", "),
      });
    ...
    default:
      return {
        ...
        title: titleCase(event.streamPath),
      };
  }
}
