...
import {
  JSONRPCMessage, McpServer, MessageExtraInfo, WebStandardStreamableHTTPServerTransport
...
server.registerTool("random_company_numbers", {
  title: "Random Company Numbers", description: "Returns random UK company numbers", inputSchema: z.object({
    count: z
      .number()
      .int()
      .min(1)
...
server.registerTool("get_stream_health", {
...
server.registerTool("get_resource_kind_json_schema", {
  ...
  if (!schema) return {
    isError: true, content: [{ type: "text", text: "resource_kind not found. Try listing resource_kinds first." }]
  }
  return {
    content: [{ type: "text", text: schema }]
  }
})
server.registerTool("list_all_resource_kinds", {
...
server.registerTool("list_stream_paths", {
...
server.registerTool("list_resource_kinds_frequencies_for_stream", {
...
server.registerTool("get_latest_sample_event", {
  ...
  if (streamPaths.has(streamPath)) {
    ...
    return {
      isError: true, content: [{ type: "text", text: "Invalid stream path. Try listing stream paths first." }]
    }
  }
})
server.registerTool("get_stream_daily_event_count", {
...
server.registerTool("get_all_time_visitor_count", {
...
server.registerTool("get_visitor_count_for_date", {
