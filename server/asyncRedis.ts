...
export const getValue: (key: string) => Promise<string | null> = async (key) => {
    // console.log(`Redis getValue('${key}')`)
    try {
        const client = createClient({host: 'redis-cache'})
        const value = await new Promise<string | null>((resolve => client.get(key, (e, r) => {
            if (e) {
                console.error('Error fetching Redis value', e)
                resolve(null)
            } else {
                logger.color(r ? 'green' : 'red').log(`Redis Cache ${r ? 'HIT' : 'MISS'} for ${key}`)
                resolve(r)
            }
        })))
...
export const setValue: (key: string, value: string) => Promise<void> = async (key, value) => {
    console.log(`Redis setValue('${key}', '${value}')`)
    try {
        const client = createClient({host: 'redis-cache'})
        await new Promise((resolve => client.set(key, value, (e, r) => {
            if (e) {
