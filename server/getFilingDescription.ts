...
export const getFilingDescription = async (req: Request, res: Response) => {
    ...
    if (typeof descriptionFormat !== 'string') // can't find description in database (rare)
        res.status(404).end()
    else {
        let formattedDescription = descriptionFormat.replace(/{([a-z_]+)}/g, (s) => description_values ? description_values[s.slice(1, s.length - 1)] || '' : '')
